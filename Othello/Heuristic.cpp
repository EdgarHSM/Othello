...
int Heuristic::eval(const Board& oth)
{
	...
	if (oth.emptySquaresLeft() >= 35) {
		return ((10 * coinParity) + (40 * (actualMobility + potentialMobility)) + (20 * corners));
	}
	else {
		if (oth.emptySquaresLeft() >= 5 && oth.emptySquaresLeft() < 35 && cornersNumberSum<4)
		{
			return ((10 * coinParity) + (20 * (actualMobility + potentialMobility)) + (40 * corners));
		}
		else
		{
			return ((50 * coinParity) + (5 * (actualMobility + potentialMobility)) + (15 * corners));
		}
	}
